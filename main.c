...
uint8_t hash_table[HASH_NUM][MAX_HASH];
uint32_t seed_list[HASH_NUM] = {353, 421, 499, 503, 619};
uint32_t shift_list[HASH_NUM] = {2, 3, 4, 5, 7};
uint8_t buffer[2048];

uint32_t hash(uint32_t hashnum, uint8_t *ptr) 
{
    uint64_t h = seed_list[hashnum];
    uint32_t shift = shift_list[hashnum];
    
    //for( ; *ptr != '\0'; ptr++)
	for( ; *ptr != '\r' && *ptr != '\n' && *ptr != '\0'; ptr++)
    {
        h = ((h << shift) + (uint32_t)(*ptr)) % MAX_HASH;
    }
    return h;
}
		...
		if (ptr != NULL) {
			...
			if (ptr != NULL) {
				...
				*/
				...
				// filtering packet by hash
				uint8_t hashflag = 1;
				for (int i = 0; i < HASH_NUM; i++) 
				{
					if (!hash_table[i][hash(i, http_host)]) 
					{
						printf("Hash: %d\n", hash(i, http_host));
						hashflag = 0;
						break;
...
int main(int argc, char **argv)
{
	...
	if (argc != 2) {
		printf("syntax : 1m_block top-1m.txt\n");
